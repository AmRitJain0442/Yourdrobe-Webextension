...
export function ProfileSetup({ requirements, productTypes, onSaved, onCancel }: Props) {
  ...
  function setFile(requirement: RequirementKey, file?: File) {
    setFiles((current) => ({ ...current, [requirement]: file }));
    setErrors((current) => {
      if (!current[requirement]) return current;
      const next = { ...current };
  ...
  return <section>
    ...
    <form aria-busy={busy} onSubmit={(event) => void save(event)}>
      <fieldset className="profile-fields-group" disabled={busy}>
        <div className="profile-fields">
          {requirements.map((requirement) => {
            const inputId = `${requirement}-photo`;
            const guidanceId = `${requirement}-guidance`;
            const errorId = `${requirement}-error`;
            return <div className="profile-field" key={requirement}>
              <label htmlFor={inputId}>{guidance[requirement].label}</label>
              <p className="profile-guidance" id={guidanceId}>{guidance[requirement].help}</p>
              <input id={inputId} type="file" accept="image/jpeg,image/png,image/webp" aria-describedby={`${guidanceId}${errors[requirement] ? ` ${errorId}` : ""}`} aria-invalid={errors[requirement] ? true : undefined} onChange={(event) => setFile(requirement, event.target.files?.[0])} />
              ...
              {errors[requirement] && <p className="error" id={errorId} role="alert">{errors[requirement]}</p>}
            </div>;
          })}
      ...
      <label className="check"><input type="checkbox" disabled={busy} checked={consent} onChange={(event) => setConsent(event.target.checked)} />I agree to browser-local profile storage and per-run transmission of required photos and optional attributes to 127.0.0.1:8001.</label>
