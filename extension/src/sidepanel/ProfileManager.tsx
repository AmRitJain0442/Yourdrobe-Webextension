...
type Props = { profile: ProfileMetadata | null; legacyImage: string | null; onChanged: () => void; onClose: () => void };
...
export function ProfileManager({ profile, legacyImage, onChanged, onClose }: Props) {
  ...
  async function replace(role: PhotoRole, file?: File) {
    ...
    try {
      ...
      onChanged();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not save that photo."); }
  }

  async function removeAsset(role: PhotoRole) {
    if (!globalThis.confirm(`Delete ${labels[role]}?`)) return;
    try { await deleteAsset(role); onChanged(); } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not delete that photo."); }
  }

  async function assignLegacy() {
    try { await assignLegacyImage(legacyRole); onChanged(); } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not assign that photo."); }
  }

  async function removeLegacy() {
    if (!globalThis.confirm("Delete this old photo?")) return;
    try { await deleteLegacyImage(); onChanged(); } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not delete that photo."); }
  }
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try { await saveAttributes(next); onChanged(); } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not save your attributes."); }
  }

  async function removeProfile() {
    if (!globalThis.confirm("Delete your complete local Yourdrobe profile?")) return;
    try { await deleteProfile(); onChanged(); } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not delete your profile."); }
  }

  return <section className="profile-manager">
    <div className="profile-heading"><h2>Manage your profile</h2><button type="button" className="secondary" onClick={onClose}>Close</button></div>
    ...
    <div className="list">{roles.filter((role) => assets[role]).map((role) => <article className="profile-asset" key={role}>
      ...
      <label>Replace photo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void replace(role, event.target.files?.[0])} /></label>
      <button type="button" className="secondary" onClick={() => void removeAsset(role)}>Delete photo</button>
    </article>)}</div>
    {legacyImage && <article className="profile-asset legacy-image"><h3>Unclassified existing photo</h3><img src={legacyImage} alt="Unclassified existing photo" /><label>Photo role<select value={legacyRole} onChange={(event) => setLegacyRole(event.target.value as PhotoRole)}>{roles.map((role) => <option key={role} value={role}>{labels[role]}</option>)}</select></label><button type="button" onClick={() => void assignLegacy()}>Assign photo</button><button type="button" className="secondary" onClick={() => void removeLegacy()}>Delete old photo</button></article>}
    <form onSubmit={(event) => void save(event)}><h3>Optional attributes</h3>{attributeGroups.map((group) => <fieldset key={group.title}><legend>{group.title}</legend>{group.fields.map((field) => <label key={field.key}>{field.label}<input name={field.key} type={field.number ? "number" : "text"} min={field.number ? "0.1" : undefined} step={field.number ? field.step ?? "1" : undefined} value={attributes[field.key] ?? ""} onChange={(event) => setAttributes((current) => ({ ...current, [field.key]: event.target.value }))} /></label>)}</fieldset>)}<button type="submit">Save attributes</button></form>
    {error && <p className="error" role="alert">{error}</p>}
    <button type="button" className="danger" onClick={() => void removeProfile()}>Delete complete profile</button>
  </section>;
}
